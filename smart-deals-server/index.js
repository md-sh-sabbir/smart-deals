...
const verifyJWTToken = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
    ...
    console.log("after docoded", decoded);
    req.token_email = decoded.email;
    next();
  });
};

const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.enlhfah.mongodb.net/?appName=Cluster0`;
...
async function run() {
  try {
    ...
    app.get("/products", async (req, res) => {
      // const projectFields = { _title: 1, price_min : 1, price_max : 1, image : 1 };
